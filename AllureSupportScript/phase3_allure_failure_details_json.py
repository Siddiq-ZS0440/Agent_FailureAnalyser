"""
Phase 3 - Open an existing Allure HTML report, capture total test count,
collect failed suite/test details, and write JSON output using XPath.

Usage:
    python phase3_allure_failure_details_json.py /path/to/Allure_Report.html [output_json_path]
...
Notes:
    ...
    - XPath locators are used for Overview/Suites navigation, suite traversal,
      failed label detection, failed branch expansion, failed leaf-test counting,
      and failure detail extraction.
"""
...
# XPath locator to identify whether the Overview section exists in the Allure report UI.
OVERVIEW_SECTION_XPATH = (
    "//a[contains(@href, '#overview') "
    "or .//*[normalize-space()='Overview'] "
    "or normalize-space()='Overview']"
)


# XPath locator fallbacks to capture total test count from Overview.
OVERVIEW_TOTAL_TEST_COUNT_XPATHS = [
    (
        "//*[contains(concat(' ', normalize-space(@class), ' '), ' statistic__item ')]"
        "[.//*[contains(translate(normalize-space(.), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'total')]]"
        "//*[contains(concat(' ', normalize-space(@class), ' '), ' statistic__count ')]"
    ),
    (
        "//*[contains(concat(' ', normalize-space(@class), ' '), ' summary-widget ')]"
        "//*[contains(concat(' ', normalize-space(@class), ' '), ' chart__caption ')]"
    ),
    (
        "//*[contains(translate(normalize-space(.), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'total')]"
...
FAILED_LEAF_TEST_CASE_NAME_RELATIVE_XPATH = ".//div[@class='node__name']"
FAILED_LEAF_PARENT_TEST_NAME_RELATIVE_XPATH = (
    "./ancestor::div[contains(concat(' ', normalize-space(@class), ' '), ' node ')]"
    "[./div[contains(concat(' ', normalize-space(@class), ' '), ' node__title ')]][1]"
    "/div[contains(concat(' ', normalize-space(@class), ' '), ' node__title ')]"
    "//div[@class='node__name']"
)
...
def get_report_url(html_file_path: str) -> str:
    ...
    if not html_path.exists():
        raise FileNotFoundError(
            ...
            "python phase3_allure_failure_details_json.py "
...
def open_overview_section_if_present(page: Page) -> bool:
...
def get_total_test_count_from_overview(page: Page) -> int:
    """Capture total test count from the Overview page using XPath locators."""
    is_overview_present = open_overview_section_if_present(page)
    ...
    for total_count_xpath in OVERVIEW_TOTAL_TEST_COUNT_XPATHS:
        total_count_elements = page.locator(f"xpath={total_count_xpath}")
        total_count_element_count = total_count_elements.count()

        for index in range(total_count_element_count):
            total_count_text = total_count_elements.nth(index).inner_text(timeout=DEFAULT_TIMEOUT_MS).strip()
...
def wait_for_failed_test_in_right_panel(page: Page, failed_test_case_name: str) -> None:
    ...
    right_test_name_xpath = (
        "//div[contains(concat(' ', normalize-space(@class), ' '), ' side-by-side__right ')]"
        "//*[contains(concat(' ', normalize-space(@class), ' '), ' test-result__name ')]"
        f"//*[normalize-space(.)={failed_test_case_name_literal}]"
    )
...
def get_failure_details_from_right_panel(page: Page, failed_test_case_name: str) -> tuple[str, str]:
...
def get_failed_leaf_parent_test_name(failed_test: Locator) -> str:
    """Get the closest failed grouping/test name above the actual failed leaf test."""
    parent_test_name = failed_test.locator(f"xpath={FAILED_LEAF_PARENT_TEST_NAME_RELATIVE_XPATH}").first

    try:
        return get_text(parent_test_name)
    except PlaywrightTimeoutError:
        return ""


def process_failed_suite(page: Page, suite_node: Locator, suite_index: int) -> tuple[str, int, list[dict]] | None:
    """Process one suite node, expand it, and collect failed leaf-test details."""
    ...
    for failed_test_index in range(failed_leaf_count):
        ...
        failed_test_name = get_failed_leaf_parent_test_name(failed_test)
        ...
        if failed_status_count > 0:
            failure_message, failure_trace = get_failure_details_from_right_panel(page, failed_test_case_name)

            failed_test_details.append(
                {
                    "suite_name": suite_name,
                    "test_name": failed_test_name,
                    "test_case_name": failed_test_case_name,
                    "status": "Failed",
            ...
            print(
                f"  Skipped test {failed_test_index + 1}: {failed_test_case_name} "
                "because text_status_failed was not found under node__leaf"
            )

    return suite_name, failed_leaf_count, failed_test_details
...
def main() -> None:
    ...
    output_json_path = sys.argv[2] if len(sys.argv) >= 3 else "allure_failure_details.json"
    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            total_test_count = get_total_test_count_from_overview(page)
            ...
            for suite_index in range(suite_node_count):
                ...
                if suite_result is not None:
                    suite_name, failed_test_count, suite_failed_test_details = suite_result
                    failed_suite_summary.append((suite_name, failed_test_count))
                    failed_test_list.extend(suite_failed_test_details)
            ...
            failure_details_json = {
                "source_html_file": Path(html_file_path).name,
                "source_html_path": str(Path(html_file_path).resolve()),
                "report_type": "allure",
