...
EXPANDED_FAILED_TEST_STATUS_BY_TEST_NAME_XPATH = (
    "//*[contains(@class, 'node__anchor')]"
    "[.//*[contains(@class, 'node__name') and normalize-space(.)=replacefailedtestname]]"
    "//span[contains(translate(@class, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'text_status_failed')]"
)


EXPANDED_FAILED_TEST_STATUS_ANYWHERE_XPATH = (
    "//span[contains(translate(@class, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'text_status_failed')]"
)
...
def click_failed_tests_and_count_expanded_failed_status(page: Page, suite_name: str) -> int:
    ...
    for failed_test_index in range(failed_test_element_count):
        ...
        failed_test_name = failed_test_element.locator(
            "xpath=.//div[@class='node__name']"
        ).first.inner_text().strip()

        failed_test_element.click()
        page.wait_for_timeout(1_000)

        failed_test_name_literal = xpath_literal(failed_test_name)
        expanded_failed_status_xpath = EXPANDED_FAILED_TEST_STATUS_BY_TEST_NAME_XPATH.replace(
            "replacefailedtestname",
            failed_test_name_literal,
        )
        failed_status_count = page.locator(f"xpath={expanded_failed_status_xpath}").count()

        if failed_status_count == 0:
            failed_status_count = page.locator(
                f"xpath={EXPANDED_FAILED_TEST_STATUS_ANYWHERE_XPATH}"
            ).count()

        if failed_status_count > 0:
            expanded_failed_status_count += 1
