...
def wait_for_failed_test_in_right_panel(page: Page, failed_test_name: str) -> None:
    """Wait until the clicked failed test name is present in right-side content."""
    ...
    right_test_name_xpath = (
        ...
        f"//*[normalize-space(.)={failed_test_name_literal}]"
    )
...
def get_failure_details_from_right_panel(page: Page, failed_test_name: str) -> tuple[str, str]:
    """Get status-details__message and status-details__trace from right-side content."""
    wait_for_failed_test_in_right_panel(page, failed_test_name)
...
def process_failed_suite(page: Page, suite_node: Locator, suite_index: int) -> tuple[str, int] | None:
    ...
    for failed_test_index in range(failed_leaf_count):
        ...
        if failed_status_count > 0:
            failure_message, failure_trace = get_failure_details_from_right_panel(page, failed_test_name)
